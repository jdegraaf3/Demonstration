...
class Graph {
    ...
    private:
        ...
        int getOrCreateNode(string node_label);
        int getNodeIdx(const string& label);
        
        vector<int> neighbors(int nodeIdx);
        vector<int> neighboringEdges(int nodeIdx);
        
        void traverseDFS(int nodeIdx, double threshold, std::set<int>& reachable);
        
        double smallest_connecting_threshold_old(string const & start_label, string const & end_label);
        
        //Alice,Bob,weight
        //David,Charlie,weight
        //Alice,David,weight
        
        // better for directed edges
        // vector<
        // vector<> a's edges   a,b
        // vector<> b's edges   b,a
        // >
